...
fullNames.push(name.first_name)
fullNames.push(name.last_name)
})
...
let firstNamesAllCaps = [];
 ...
 firstNamesAllCaps = runners.map(name => name.first_name.toUpperCase());
...
let total = runners.reduce((acc, runner) =>  
   acc + runner.donation, 0);
...
// Problem 1
//Use .forEach to print the emails of all the donors in order to send thank you mails to them
let mails = [];
...
// Problem 2
//The refreshments ran out during the event and one of the donors by name Toni offerred to order more. You want to get her details in order to send them a special thank you mail
let nicePerson = [];
nicePerson = runners.filter(nice => {
  return nice.first_name === "Yevette" || nice.last_name === "Yevette";
});
console.log(nicePerson);

// Problem 3
//How much is 7% of each donation

const tax = runners.map(pay => pay.donation * 0.07)
console.log(tax)